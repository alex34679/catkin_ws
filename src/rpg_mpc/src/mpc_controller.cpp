...
MpcController<T>::MpcController(
  ...
  pub_predicted_trajectory_marker_ = 
...
bool MpcController<T>::publishPrediction(
  ...
  visualization_msgs::Marker path_marker_msg;
  ...
  path_marker_msg.header.frame_id = "world"; 
  path_marker_msg.header.stamp = ros::Time::now();
  path_marker_msg.ns = "state_positions";
  path_marker_msg.id = 0;
  path_marker_msg.type = visualization_msgs::Marker::SPHERE_LIST; // 显示一系列球体
  path_marker_msg.action = visualization_msgs::Marker::ADD;
  
  path_marker_msg.pose.orientation.w = 1.0; // 单位四元数，表示无旋转
  path_marker_msg.scale.x = 0.1; // 球体的尺寸
  path_marker_msg.scale.y = 0.1;
  path_marker_msg.scale.z = 0.1;
  path_marker_msg.color.r = 1.0;
  path_marker_msg.color.g = 0.0;
  path_marker_msg.color.b = 0.0;
  path_marker_msg.color.a = 1.0; // 不透明度
  ...
  for (int i = 0; i < kSamples; i++) {
    ...
    path_marker_msg.points.push_back(p);
    path_msg.poses.push_back(pose);
  }

  pub_predicted_trajectory_.publish(path_msg);
  pub_predicted_trajectory_marker_.publish(path_marker_msg);
